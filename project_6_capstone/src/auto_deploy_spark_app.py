...
config.read_file(open(path + '/spark_app/dl.cfg'))
...
class DeplySparkApp:
    ...
    def run(self, app_files, arguments):
        ...
        self.make_tarfile(                                  #Create a tar.gz file for spark app
            ...
            app_files,
            self.app_dir)
        self.upload_files(                                  #Upload bash files and app files to s3
            s3,
            self.my_bucket, 
        ...
        self.create_cluster_run_job(                        #Create a cluster and run the spark app
            c,
            arguments)        
        self.wait_till_termindate(c)                        #Print states until the cluster termindates
    ...
    def make_tarfile(self, output_filename, file_list, app_dir):
        '''
        ...
        Pparam:
            ...
            add_dir         : Dir of the spark app
        '''
        with tarfile.open(app_dir + '/' + output_filename, "w:gz") as tar:
            for file in file_list:
                tar.add(app_dir + '/' + file, arcname= file)

                
    def upload_files(self, s3, buck_name, script_folder, app_dir, tarfile_name):
        """
        ...
        param   s3            : s3 resource.
                ...
                app_dir       : dir of the spark app stored.
                tarfile_name  : name ofthe tar file.
        return:
        """
        # Shell file: setup (download S3 files to local machine)
        s3.Object(buck_name, script_folder + '/setup.sh')\
          .put(Body=open(app_dir + '/setup.sh', 'rb'), ContentType='text/x-sh')
        # Shell file: Terminate idle cluster
        s3.Object(buck_name, script_folder + '/terminate_idle_cluster.sh')\
          .put(Body=open(app_dir + '/terminate_idle_cluster.sh', 'rb'), ContentType='text/x-sh')
        # Compressed Python script files (tar.gz)
        s3.Object(buck_name, script_folder + '/' + tarfile_name)\
          .put(Body=open(app_dir + '/' + tarfile_name, 'rb'), ContentType='application/x-tar')
